---
title: Origin, Referer (Referrer) and Referrer-Policy
description: Origin, Referer (Referrer) and Referrer-Policy
last_update:
...
為了等等方便測試，我們一樣先建立一個簡單的 Node.js HTTP server，所有 path 跟 method 都統一回傳 `ok` 字串就好
...
downgrade（https > http 或 https > file） 情境就不送 `Referer`，反之就送完整 URL
