...
## Node.js http 實測 HTTP/1.1 pipeline
...
但為何 pipeline 總共花了 8 秒才完成呢？上面 [Safe Methods](#safe-methods) 有提到，RFC 並沒有規定 server 一定要平行處理，Node.js `http.Server` 是一個一個處理，這邊總共 1 + 2 = 3 秒
...
Node.js `http.Server` 預設的 `keepAliveTimeout`，參考 [官方文件](https://node.js.org/api/http.html#serverkeepalivetimeout) 是 5 秒
